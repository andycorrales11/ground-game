...
`data/keepers.json` and `data/trades.json` describe one particular league's keepers and traded picks; both are optional and both are gitignored along with the rest of `data/`. See League structure below for their shapes.
...
- **Simulation** — a `PickBook` (see League structure below) owns both the user's pick numbers and the team on the clock; CPU picks are generated by `simulate_cpu_pick`.
...
Simulation-mode draft order lives in **one** place, `backend/draft_order.py`. It used to be the same `current_round % 2 == 0` arithmetic copied into five (`draft_manager_service` ×4, `vbd_service.simulate_to_next_turn`), and the copies were not equivalent — `process_auto_pick_helper`'s used the seat index against live mode, which indexes `teams_list` by roster id, so it auto-picked onto the wrong roster in any league whose roster ids were not in seat order.
...
### League structure: draft order, keepers, traded picks

`backend/draft_order.py` holds `DraftOrder` (the seating plan) and `PickBook`
(who owns each pick, and which are already spent). `backend/keepers.py` reads the
two JSON files that describe one particular league.

**The snake need not start in round two.** `DraftOrder.snake_from` is the first
round that runs backwards; 2 is the definition of a plain snake and is the
default, so every league that predates the setting drafts exactly as before. The
UnderAchievers league runs rounds 1–3 in order and snakes from round 4, which is
`snake_from=4`. The rule that a round reverses when `(round - snake_from)` is
even follows from the turn rather than from parity: the last straight round ends
on the final seat, so the first snaking round has to begin there.

Round 2 is the *only* round where `snake_from=4` differs from a plain snake — 
which is exactly what made the old parity test dangerous. It is wrong in a way
that looks right for fourteen rounds out of fifteen.

**Picks are named by seat, never by position within the round.** A keeper is
`{"player": ..., "round": 5, "pick": 10}` where `pick` is the manager's draft
slot, so manager 10 is "pick 10" in every round and filling the file in never
requires knowing which way that round runs. Note this is *not* how Sleeper labels
picks: Sleeper writes `round.position-in-round`, so seat 1's second-rounder shows
there as 2.12. `DraftOrder.pick_index` does the conversion, and rejects a `pick`
outside the seat range with an error that says so.

**Keepers are applied before the draft opens, all at once.** That is what is
actually true — everybody knows who was kept — so no valuation should ever see a
kept player available, and `Team.picks_made` starts on the right number for the
CPU's endgame logic. The clock then never rests on a kept pick: `next_open_pick`
steps over them, which is why the real league's draft opens on pick 4 and why
every pick path calls `_advance_clock` rather than incrementing.

**An unresolved keeper refuses to start the draft.** Everything else about these
files is soft — no files, no keepers, fine — but a keeper that silently failed to
apply would leave the player on the board *and* leave his pick live, two errors
compounding for the rest of the draft with nothing downstream able to notice.
The same goes for a keeper whose stated `manager` disagrees with who owns that
pick after trades: the sheet already knows the answer, so a mismatch means either
the keeper's row or the trade is wrong.

**The two files compose, and that is the point of keeping them separate.** A
draft sheet writes a kept player in the seat that originally held the pick and
notes the new owner beside it, so Caleb Williams sits in Adrian's round-5 row and
belongs to Andy. `build_pick_book` applies trades first and then reads each
keeper's owner off the pick.

**VONA has to be handed the real sequence, not a count.** `open_picks_between`
returns the team indices of the picks actually made in a span, and
`simulate_to_next_turn` plays exactly those. A kept pick inside the span is
already made, so simulating it would take a player off the board who was never
going anywhere and overstate what waiting costs; a traded pick belongs to a
...
`rounds_remaining` is the team's **own** remaining picks, passed in as `picks_remaining` from `PickBook.picks_remaining`. It falls back to `total_rounds - team.picks_made` when no book is available, which is the same number only in a league where nobody trades: a manager who traded three picks away has twelve in a fifteen-round draft, and on the derived count never reached the final-rounds window at all — measured on the real league, that was one team finishing with no kicker and no defense.
